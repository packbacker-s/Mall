...
public class OrderServiceImpl implements IOrderService {
    ...
    public ResponseVo<OrderVo> detail(Integer uid, Long orderNo) {
        ...
        if (order == null || !order.getUserId().equals(uid)) {
            return ResponseVo.error(ResponseEnum.ORDER_OT_EXIST);
        }
    ...
    public void paid(Long orderNo) {
        Order order = orderMapper.selectByOrderNo(orderNo);
        if (order == null) {
            throw new RuntimeException(ResponseEnum.ORDER_OT_EXIST.getDesc() + "订单id：" + orderNo);
        }
    ...
    private OrderVo buildOrderVo(Order order, List<OrderItem> orderItemList, Shipping shipping) {
        ...
        BeanUtils.copyProperties(order, orderVo);//把order对象的属性copy到orderVo
    ...
    private Order buildOrder(Integer uid,
        ...
        BigDecimal payment = orderItemList.stream()
                .map(OrderItem::getTotalPrice)//对getTotalPrice做累加，初始值是zero
